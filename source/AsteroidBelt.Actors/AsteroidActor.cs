...
public sealed class AsteroidActor : ReceiveActor
{
    ...
    private void AliveBehavior(AsteroidMotion motion, IActorRef mediator)
    {
        ...
        Receive<AsteroidState>(_ =>
        {
            if (_.AsteroidId != Self.Path.Name && _.Weight >= weight)
            {
                ...
                {
                    logger.Info($"Asteroid {Self.Path.Name} is destroyed by {Sender.Path.Name}");
        ...
        Receive<SubscribeAck>(ack =>
        {
            ...
            {
                logger.Info($"subscribing Asteroid {Self.Path.Name} to topic {ack.Subscribe.Topic}");
            }
        });

        Receive<UnsubscribeAck>(ack =>
        {
            logger.Info($"unsubscribing Asteroid {Self.Path.Name} from topic {ack.Unsubscribe.Topic}");
        });
    }
